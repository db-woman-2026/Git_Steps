...
- 수강생 프로젝트 경로 예시는 `$HOME\dongbu\git-practice`를 사용하고 공백 경로, UTF-8, 줄바꿈 상태를 확인합니다.
...
- 각 단계는 시작 상태, 관찰할 변화, 실행 절차, 예상 결과, 완료 기준을 모두 적습니다.
- Git 명령을 제시할 때는 실행 전 branch와 working tree 상태를 먼저 확인합니다.
- stage 전 `git diff`, stage 후 `git diff --staged`, commit 후 `git status`와 `git log`를 구분해 보여줍니다.
- 오류 예제에는 원인 확인 명령, 중단 또는 복구 명령, 복구 후 확인 명령을 함께 둡니다.
- 강사용 `step-N` 브랜치와 수강생 개인 저장소의 `main`을 혼동하지 않도록 출발 위치를 명시합니다.
- 수강생이 교안 저장소의 ZIP이나 clone에 의존하지 않고 연습 파일을 직접 만든 뒤 `git init`부터 시작하도록 안내합니다.
- 예상 출력의 commit hash, 사용자명, 저장소 URL처럼 환경마다 달라지는 값은 예시라고 밝힙니다.
- 새 diff를 넣으면 수강생의 직전 단계 파일에 실제로 적용되는지 확인합니다.
- 새 단계를 추가하면 실습 구성의 선수 단계, 완료 기준, 독립 확인 항목도 갱신합니다.
