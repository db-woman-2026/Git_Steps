...
`Permission denied (publickey)`가 나오면 공개 키가 현재 로그인한 GitHub 계정에 등록됐는지 확인합니다. 설치부터 다시 진행하지 말고 [초기 설정](./lecture/requirements.md) <span class="print-reference" data-print-reference="true">(Git · Windows 11 x64, Git, GitHub 초기 설정 · 1. Windows Terminal 설치)</span>의 기존 키 확인, 공개 키 등록, SSH 연결 테스트를 차례로 다시 확인합니다.
