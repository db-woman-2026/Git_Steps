...
이번 단계에서는 지금까지 배운 흐름을 반복합니다.

새로운 개념을 많이 추가하지 않습니다. 대신 브랜치를 만들고, 파일을 수정하고, commit하고, `main`으로 돌아와 merge하는 흐름을 두 번 반복합니다.

반복은 중요합니다. Git은 설명을 한 번 듣는 것보다 같은 흐름을 손으로 여러 번 해볼 때 훨씬 빨리 익숙해집니다.
...
그래프는 조금 복잡해 보일 수 있습니다. 지금은 모든 선을 해석하려고 하지 않아도 됩니다. 중요한 것은 다음 흐름을 스스로 반복했다는 점입니다.
