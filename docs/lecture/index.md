...
3. 문서의 예상 diff는 수강생 저장소에서 commit 전에 `git diff`를 실행했을 때 보게 되는 기준입니다.
4. `+` 줄은 새로 추가되거나 바뀐 내용이고, `-` 줄은 사라지거나 교체되는 내용입니다.
5. commit 후에는 Source Control 변경 목록이 비어 있어야 합니다.
6. conflict 단계에서는 Git이 자동으로 고르지 못한 내용을 사람이 직접 결정합니다.
