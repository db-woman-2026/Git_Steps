...
| `step-14` | [lecture/step-14.md](./step-14.md) | 최종 실습 결과를 내 GitHub 저장소에 push하고 온라인에서 기록을 확인합니다. |
...
4. `step-1`부터 예상 diff 바로 위의 `git diff` 또는 `git diff --staged` 명령을 실행해 결과를 비교합니다.
5. `+` 줄은 새로 추가되거나 바뀐 내용이고, `-` 줄은 사라지거나 교체되는 내용입니다.
6. `index` 뒤의 짧은 hash는 파일 내용과 Git 저장소 형식에 따라 달라질 수 있는 예시입니다. 파일 경로, `@@` 범위, `+`/`-` 줄을 기준으로 확인합니다.
7. commit 후에는 Source Control 변경 목록이 비어 있어야 합니다.
8. conflict 단계에서는 자동으로 합쳐지지 않은 내용을 사람이 직접 결정합니다.
