...
이번 단계에서는 두 개의 브랜치를 만듭니다.

첫 번째 브랜치는 `practice/profile.md`를 수정합니다. 두 번째 브랜치는 `practice/goal.md`를 수정합니다. 두 브랜치가 서로 다른 파일을 고치기 때문에 Git은 보통 자동으로 합칠 수 있습니다.

이번 단계가 끝나면 `main`에 두 브랜치의 변경이 모두 들어와야 합니다.
...
```bash
git switch main
git merge branch/edit-profile
git merge branch/edit-goal
```
...
둘 다 conflict 없이 끝났다면 정상입니다. 화면 결과가 달라도 "서로 다른 파일을 고쳤기 때문에 Git이 자동으로 합쳤다"는 점이 핵심입니다.
